...
function startFallSequence() {
    ...
    function fallStep() {
        if (fallLevel < maxFallLevels) {
            ...
            // Player starts at top of screen
            ...
            player.jumpHeight = -30; // Use a room number that doesn't show text
...
class Player {
    ...
    update() {
        ...
        } else if (this.position.x < -100 && this.jumpHeight < 0) {
            this.jumpHeight += 2;
        ...
        if (this.jumpHeight === 0 && this.position.x < 0) {
            this.position.x = 0;
        }
        if (this.jumpHeight === roomLimit && this.position.x > canvas.width - 100) {
...
function renderLevelText() {
    ...
    if (canvasColor !== '#add8e6') {
        context.fillText(`${Math.abs(player.jumpHeight) / 2}`, canvas.width / 2, canvas.height / 2);
    }

    context.font = '50px Arial';
    context.fillStyle = fontColor;

    context.fillText(currentMessages[player.jumpHeight] || '', canvas.width / 2, canvas.height - 50);

    if (player.jumpHeight === -2 && canvasColor === '#e0e0e0') {
        context.font = '30px Arial';
        context.fillText('[]', canvas.width / 2, canvas.height - 115);
    }

    if (player.jumpHeight === -20 && canvasColor !== 'black') {
        context.font = '30px Arial';
...
window.addEventListener('keydown', (e) => {
    switch (e.key) {
        ...
        case 'd':
            ...
            break;
        case 's':
            if (player.jumpHeight === -2) player.velocity.y = 10;
            // Check if in final leftmost room of sky room (jumpHeight = 0, anywhere in room)
            if (player.jumpHeight === 0 && canvasColor === '#add8e6') {
                startScreenShake();
            }
            break;
        case 'Enter':
            if (player.jumpHeight === -2) {
                canvasColor = 'black';
                playerColor = 'white';
                player.color = playerColor;
                roomLimit = -24;
